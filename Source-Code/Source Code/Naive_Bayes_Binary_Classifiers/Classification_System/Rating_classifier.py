...
class Rating_classifier:
    ...
    def __init__(self):
        ...
        self.db = db
        self.cur = cur 
        self.train ,self.rating_dict , self.test , self.senti_dict , self.senti_pos_dict ,self.senti_neg_dict ,self.present_simple_dict ,self.past_simple_dict ,self.future_dict ,self.present_con_dict = Rating_classifier.classifier_fetch_data(self)
        print('Training the Classifier (Rating Classifier)')
    ...
    def classifier_fetch_data(self):
        ...
        for row in self.cur.fetchall() :
            ...
            senti_neg_dict.update({decoded_row: senti_neg})
            decoded_row = str(decoded_row.decode('utf-8',errors = 'ignore'))
            test.append((decoded_row, 'Not_rating'))
