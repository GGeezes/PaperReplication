...
class UserExperience_classifier:
    ...
    def __init__(self):
        ...
        self.db = db
        self.cur = cur 
        self.train ,self.rating_dict , self.test , self.senti_dict , self.senti_pos_dict ,self.senti_neg_dict ,self.present_simple_dict ,self.past_simple_dict ,self.future_dict ,self.present_con_dict = UserExperience_classifier.classifier_fetch_data(self)
        print('Training the Classifier (User Experience Classifier)')
    ...
    def probability(self,text):
        #prob_dist = self.classifer.prob_classify("Every time I try to get on the new story, the screen goes black and the I&#39;m back on my home screen! Fix it!!!")
        #print prob_dist.max()
        prob_dist = self.classifer.prob_classify(text)
        print "probaility of having a UserExperience review" 
        print prob_dist.prob("UserExperience")
        print "classified as :"
        print prob_dist.max()


def main():
    Classifier = UserExperience_classifier()
